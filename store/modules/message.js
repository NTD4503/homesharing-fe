...
export default {
  ...
  actions: {
    async fetchConversations({ commit }, userId) {
      const res = await this.$axios.get(`/api/v1/chat/conversations/${userId}`);
    ...
    async createConversation({ commit }, { user1_id, user2_id }) {
      const res = await this.$axios.post("/api/v1/chat/conversations", {
        user1_id,
        user2_id,
      });
    ...
    async sendMessage(
      { commit },
      { conversation_id, sender_id, text, image_url = "" }
    ) {
      const res = await this.$axios.post("/api/v1/chat/messages", {
        conversation_id,
        sender_id,
        text,
        image_url,
    ...
    subscribeToConversation({ state, commit }, { conversationId, pusher }) {
      if (!pusher) {
        console.warn(
          "Pusher chưa được khởi tạo, truyền pusher instance vào action"
        );
        return;
      }
